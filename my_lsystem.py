...
class LSystem:
    ...
    def __init__(self, axiom, rules):
        ...
        """
        self.axiom = axiom #axiom is an attribute of the LSystem class
        self.rules = rules #rules is an attribute of the LSystem class
        self.output = axiom #output is an attribute of the LSystem class

    @property #property is a decorator that allows us to define a method that can be accessed like an attribute (alphabet is an attribute of the LSystem class)
    def alphabet(self):
        """
    ...
    def iterate(self, n=1):
        ...
        """
        for i in range(n):
            next_output = self._iterate_once() #_iterate_once is a private method that applies the production rules to the current output once
            self.output = next_output #output is an attribute of the LSystem class
            print(f"Output after {i + 1} iteration(s): {self.output}") #print the output after each iteration
        final_output = self.output #final_output is an attribute of the LSystem class
        self._reset_output()
        return final_output
...
def create_and_show_music21_score(music21_chords):
    ...
    # Save the score to a file instead of displaying it
    score.write('midi', fp='l_system_chord_progression.mid')
    print("Score saved as 'l_system_chord_progression.mid'")
    print("You can open this file with any music player or notation software!")
